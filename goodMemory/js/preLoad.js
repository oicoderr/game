...
var gameName = null,gameManifest,
  ...
  positiveTiming = null, //后端接受正计时值stopTime
...
function toServerData(scoreData){
  if(scoreData){
    ...
    show.gameReward( reward, gameAllConfig.rewardPosition, resultAward, 38, "rgba(60,60,60,1)" );
    // $.ajax({
    //   type:"POST",
...
var show = {
  ...
  gameTime:function(bitmap,x,y,x1,y1,text,size,color){//x,y:bitmap 位置 ;x1,y1:文字 位置
    if(!text)text = "00:00";
    if(!color)color = "#ffffff";
    if(!size) size = "36";
    if(!bitmap){
